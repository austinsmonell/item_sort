...
Two windows come up. The MuJoCo viewer shows the machine; the control panel has
  ...
  * per-axis Kp/Kd/Ki, force limit and dry-friction sliders, so the loop and the
    drivetrain it is fighting can both be trimmed while it runs
...
class ControlPanel:
    ...
    def __init__(self, pid, robot=None):
        ...
        # --- gains, force limit and axis friction: one column per axis ---
        ...
        for axis, label in enumerate(AXIS_LABELS):
            ...
            # The force limit tops out at MAX_FORCE: the actuator ctrlrange clips
            # anything beyond it anyway. "friction" is the axis's dry friction,
            # i.e. what the drivetrain costs before the axis moves at all -- the
            # knob that makes a well-tuned PID stick short of its setpoint.
            for key, text, value, top in (
                    ("Kp", "Kp", KP[axis], KP_MAX[axis]),
                    ("Kd", "Kd", KD[axis], KD_MAX[axis]),
                    ("Ki", "Ki", KI[axis], KI_MAX[axis]),
                    ("Fmax", f"max effort ({unit})",
                     FORCE_LIMIT[axis], MAX_FORCE[axis]),
                    ("Fric", f"friction ({unit})",
                     AXIS_FRICTION[axis], AXIS_FRICTION_MAX[axis])):
